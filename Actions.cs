...
namespace Yarn.GodotEngine
{
	...
	public class Actions : ICommandDispatcher
	{
		internal class CommandRegistration : ICommand
		{
			...
			public CommandRegistration(string name, MethodInfo method)
			{
				...
				else
				{
					...
					throw new ArgumentException($"Cannot register method {GetFullMethodName(method)} as a command: instance methods must declared on {nameof(Node)} classes.");
				}
			...
			public enum CommandType
			{
				...
				/// return <see cref="void"/> or <see cref="Task"/>.)
				/// </summary>
				Invalid,
			}
			...
			internal CommandDispatchResult Invoke(DialogueRunner dispatcher, List<string> parameters, out Task commandTask)
			{
				object target;
				...
				if (DynamicallyFindsTarget)
				{
					...
					var node = GodotUtility.GetNode(nodeName);
					...
					if (node == null)
					{
						...
						return new CommandDispatchResult
						{
							Message = $"No game object named \"{nodeName}\" exists",
					...
					target = GodotUtility.GetChild(node, DeclaringType);
		...
		public void AddCommandHandler(string commandName, Func<Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1>(string commandName, Func<T1, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1, T2>(string commandName, Func<T1, T2, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1, T2, T3>(string commandName, Func<T1, T2, T3, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1, T2, T3, T4>(string commandName, Func<T1, T2, T3, T4, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1, T2, T3, T4, T5>(string commandName, Func<T1, T2, T3, T4, T5, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);

		public void AddCommandHandler<T1, T2, T3, T4, T5, T6>(string commandName, Func<T1, T2, T3, T4, T5, T6, Task> handler) => AddCommandHandler(commandName, (Delegate)handler);
		...
		CommandDispatchResult ICommandDispatcher.DispatchCommand(string command, out Task commandTask)
		{
			...
			if (_commands.TryGetValue(commandPieces[0], out var registration))
			{
				...
				return registration.Invoke(DialogueRunner, commandPieces, out commandTask);
			}
			else
			{
				commandTask = default;
		...
		private class LibraryRegistrationProxy : IActionRegistration
		{
			...
			public void AddFunction(string name, Delegate implementation)
			{
				...
				GD.Print($"Registered function {name} to method {implementation.Method.DeclaringType}.{implementation.Method.Name}");
				library.RegisterFunction(name, implementation);
			}
